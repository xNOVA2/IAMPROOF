  ...
  return (
      ...
      <TableBody>
        {TableData.map((row: TableRowData, index) => (
          <TableRow key={index}>
                  ...
                  <Dialog>
                    ...
                    {isSecurityPage ?
                      ...
                      <UserDialog
                        ...
                        additionalData={{
                          "EMAIL": row.email,
                          "SUBSCRIPTION STATUS": row.isActive ? "ACTIVE" : "DEACTIVATED",
            ...
            <TableCell>
              <div >
                {isSecurityPage ? row.SecondaryEmail || '-' : <div className="bg-[#439A86] text-start py-1 inline-block p-2 font-bold text-white rounded-full text-md">{row.isActive ? "ACTIVE" : "DEACTIVATED"}</div>}
              </div>
            </TableCell>
            <TableCell className="">
              <div className={`${row["2-Factor"] ? 'bg-[#439A86] text-white ' : null} text-start py-1 inline-block p-2 font-bold rounded-full text-md`}>
                {isSecurityPage ? row["2-Factor"] || '-': row.role?.toUpperCase() }
            ...
            <TableCell className="">{isSecurityPage ? <BlockToggle   userId={row._id} isActive={row.isActive!}/> : row.dataUsage || '-'}  </TableCell>
          </TableRow>
        ))}
    ...
    function ManageAccount({index,userId}:{index:number,userId:string}) {
      ...
      const toggleBlockUser = async (status:boolean) =>{
            ...
            if(data.success) toast.success(`${status ? ' User Account has been unlocked sucessfully' : 'User Account has been locked successfully'}  `)
      } 

      return (<div className="flex justify-center pt-10 gap-5">
                          <p key={index} className="border-[#BCD8C1] border-2 flex transform items-center rounded-lg px-10 py-3 text-center text-[#439A86] font-bold text-lg transition-colors duration-300 bg-gray-100 ">
                            <span className="mx-2 text-sm">RESET PASSWORD</span>
                          </p>
                          <p key={index} className="border-[#BCD8C1] border-2 flex transform items-center rounded-lg px-10 py-3 text-center text-[#439A86] font-bold text-lg transition-colors duration-300 bg-gray-100 cursor-pointer" onClick={()=>toggleBlockUser(false)}>
                            <Image src={'/assets/lock.png'} alt="Lock Account" width={25} height={25} />
                            <span className="mx-2 text-sm"> LOCK ACCOUNT</span>
                          </p>
                          <p key={index} className="border-[#BCD8C1] border-2 flex transform items-center rounded-lg px-10 py-3 text-center text-[#439A86] font-bold text-lg transition-colors duration-300 bg-gray-100  cursor-pointer"  onClick={()=>toggleBlockUser(true)}>
                            <Image src={'/assets/unlock.png'} alt="Unlock Account" width={25} height={25} />
